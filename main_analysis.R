...
#####################################################
# ===================================================
# Hierarchical linear models (HLM) analysis (Table 2)
# ===================================================
#####################################################
...
#################################################################
# ===============================================================
# Robustness Check (Table A6): Ordinal Logistic Regression Models
# Using the pre-standardization dataset (df)
# ===============================================================
#################################################################
df_ordinal <- df %>%
  mutate(
...
# Comparing the seven ordinal regression models 
...
# using cluster-robust standard errors (Model 7)
# ============================================================
...
# Calculate estimates, robust SEs, z statistics, and p-values
est <- b[terms_keep]
...
######################################################################################
# ====================================================================================
# Robustness Check (Table A10): Adding the intercept for courses ("+ (1 | course_id)")
# ====================================================================================
######################################################################################
...
###############################################################################
# =============================================================================
# Robustness Check (Table A11): Results of HLM with Student Major Fixed Effects
# =============================================================================
###############################################################################
...
# Retained vs. Excluded Sample Comparison
# =======================================
##########################################

# Descriptive statistics for course grade and the procrastination index
# are based on variables prior to standardization.

# This dataset corresponds to step2_df in preprocessing.R.
full_sample_with_grades <- read_csv('./cleaned_data/full_dataset_with_course_grades.csv') 
...
# Identify Retained and Excluded Student-Course Observations
retained_keys <- df %>%
  distinct(student_id, course_id) %>%
  mutate(sample_status = "Retained")

# Merge retained-sample indicators with the broader comparison sample.
# Observations not found in the final analytic sample are classified as excluded observations.
comparison <- full_sample_with_grades %>%
  select(
...
# ====================================================================
# Table A1. Comparison of Retained and Excluded Sample Characteristics
# ====================================================================

# ------------------------------------------------------------
# A1-1. Continuous Characteristics
...
# ------------------------------------------------------------
# A1-2. Cohen's d for Continuous Variables
# ------------------------------------------------------------
# Cohen's d is calculated as:
# Retained sample - Excluded sample
...
cohens_d_manual <- function(data, variable) {
  ...
  x_retained <- x_retained[!is.na(x_retained)]
  
  x_excluded <- x_excluded[!is.na(x_excluded)]
...
d_grade <- cohens_d_manual(comparison, grade)

d_procra <- cohens_d_manual(comparison, procra_rank)

print(d_grade)
print(d_procra)

# ------------------------------------------------------------
# A1-3. Demographic Characteristics
# Unique-student level
# ------------------------------------------------------------
# A retained student is defined as a student with at least one
# student-course observation included in the final analytic sample.
#
# An excluded student is defined as a student who appears in the
# comparison sample but has no student-course observations included
...
student_demo <- comparison %>%
  ...
  left_join(student_status, by = "student_id") %>%
  mutate(
    sample_status = factor(sample_status, levels = c("Retained", "Excluded"))
  )
...
gender_desc <- student_demo %>%
  count(sample_status, female_cat) %>%
  group_by(sample_status) %>%
...
# ------------------------------------------------------------
# A1-4. Cramer's V for Categorical Variables
# ------------------------------------------------------------

# Cramer's V is used to quantify differences in the distribution
# of categorical characteristics between retained and excluded samples.
...
# ------------------------------------------------------------
# A1-5. Primary-Major STEM Classification
# Student-course observation level
# ------------------------------------------------------------
...
# Cramer's V for retained/excluded differences
# in primary-major STEM classification.
